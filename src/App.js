...
class TrelloTaskBoard extends Component {
  ...
  componentDidMount(){
    let sampleLists =  [
      ...
      },
      {
        ...
        items: [
          {
            description: "Stude and understand Googl map Integration",
            taskName: "Study task",
            id: '381'
          },
          {
  ...
  updateLists = (destinationListIndex,event) =>
  {
    ...
    if (typeof taskDetails === "object" && !isNaN(taskDetails.listIndex) && taskDetails.draggedTask.id ) {
      ...
      console.warn('Could not get necessary Task details from Draf event');
    }

    // Adds dragged Task to new Destination List

    if (!isNaN(destinationListIndex) && typeof taskDetails.draggedTask === "object" && taskDetails.draggedTask.taskName && taskDetails.draggedTask.description )  {
  ...
  render(){
    ...
    return (
      <div className="App">
        ...
        <Modal title={this.state.message} action={this.addItemGeneric.bind(this)} actionName="Save" id="AddCard" ref={this.modalRef}>
            <div id="form">
                ...
                <form name="AddcardForm" >
                    ...
                    <div className="row" style={{ 'display': this.state.contextToAdd === 'TaskList' ? 'block' : 'none' }}>
                        Descriptiion :<input type="text" name="taskDescription" value={this.state.albumId} onChange={this.handleInputChange} className="form-control" placeholder="Enter breif description" required/>
                    </div>
